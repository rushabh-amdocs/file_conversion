...
def adjust_01_level(copybook_content):
    ...
    return '\n'.join(adjusted_lines)
def process_copybooks(copybook_name, output_path):
    ...
    header_lines = [
        ...
        f"        SELECT {os.path.splitext(os.path.basename(copybook_name))[0]} ASSIGN TO RA-GO99."
    ]
...
def main():
    ...
    # Process all files in the copybook directory
    copybook_files = glob.glob(os.path.join(copybook_path, '*'))
    for copybook_name in copybook_files:
        process_copybooks(copybook_name, expfd)
