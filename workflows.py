    ...
    try:
        ...
        for input_file_name in input_files:
            ...
            try:
                ...
                # Display dowitcher stdout output
                if result.stdout:
    ...
    if request.method == 'POST':
        ...
        if not os.path.exists(path_file):
            ...
            try:
                ...
                output = f'Error reading path.txt: {e}'
                success = False
            
            if output is None:  # No error yet
...
<div class="container">
    ...
    <div class="info-box">
        ...
        <p>This tool updates the application name and base path in all paths in path.txt. It will:</p>
        <ul style="color: var(--text-secondary); margin-left: 20px;">
            <li>Update the application name across all configured paths</li>
            <li>Update the base path (e.g., C:/File_conversion) for all directories</li>
    ...
    <form method="post" id="pathForm">
        <div class="form-group">
            ...
            <input type="text" id="base_path" name="base_path" value="{{ base_path }}" required 
                   placeholder="Enter base path (e.g., C:/File_conversion, D:/Projects/Conversion)">
...
def update_paths_page():
    ...
    success = False
    app_name = ''
    base_path = 'C:/File_conversion'
    current_app = 'occ'  # Default value
    current_base = 'C:/File_conversion'  # Default value
    ...
    try:
        ...
        if os.path.exists(config_file):
            with open(config_file, 'r') as f:
                content = f.read()
                # Try to find current app name and base path from paths
                import re
                # Match pattern like C:/File_conversion/occ/input
                match = re.search(r'([A-Za-z]:/[^/]+)/([^/]+)/(?:input|output|FF_FILES)', content)
                if match:
                    current_base = match.group(1)
                    current_app = match.group(2)
                    base_path = current_base  # Set base_path to current value for form display
    except:
        pass
            ...
            try:
                ...
                if not re.match(r'^[a-zA-Z0-9_-]+$', app_name):
                    ...
                    if not os.path.exists(config_file):
                        ...
                        # Replace base path and application name
                        updated_lines = []
                        changes_count = 0
                        for line in lines:
                            if '=' in line and (current_app in line or current_base in line):
                                # Get the key and value parts
                                key, value = line.split('=', 1)
                                original_value = value
                                
                                # Replace base path first (e.g., C:/File_conversion -> D:/NewPath)
